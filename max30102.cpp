...
void MAX30102::max30102_init(enum Mode mode)
{
    ...
    else
    {
        ...
        // writeRegister(fd, REG_MODE_CONFIG, SPO2_MODE_TO_REG_MODE_CONFIG);

        // writeRegister(fd, REG_FIFO_WR_PTR, SPO2_MODE_TO_REG_FIFO_WR_PTR);
        // writeRegister(fd, REG_OVF_COUNTER, SPO2_MODE_TO_REG_OVF_COUNTER);

        // writeRegister(fd, REG_FIFO_RD_PTR, SPO2_MODE_TO_REG_FIFO_RD_PTR);
        // writeRegister(fd, REG_INTR_ENABLE_1, SPO2_MODE_TO_REG_INTR_ENABLE_1);
        // writeRegister(fd, REG_INTR_ENABLE_2, SPO2_MODE_TO_REG_INTR_ENABLE_2);

        // writeRegister(fd, REG_FIFO_CONFIG, SPO2_MODE_TO_REG_FIFO_CONFIG);
        // writeRegister(fd, REG_SPO2_CONFIG, SPO2_MODE_TO_REG_SPO2_CONFIG);
        // writeRegister(fd, REG_LED1_PA, SPO2_MODE_TO_REG_RED_LED);
        // writeRegister(fd, REG_LED2_PA, SPO2_MODE_TO_REG_IR_LED);
        // writeRegister(fd, REG_PILOT_PA, SPO2_MODE_TO_REG_PILOT_PA);
    }
}

bool MAX30102::writeRegister(uint8_t max30102_fd, uint8_t reg, uint8_t add)
{
    uint8_t config[2];
    config[0] = reg;
    config[1] = add;
